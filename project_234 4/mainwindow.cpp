...
void MainWindow::loadCSV(const QString &filePath, std::vector<std::vector<double>> &data, std::vector<QStringList> &originalData) // Изменено
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox::warning(this, "Error", "Could not open file");
...
void MainWindow::on_pushButtonDisplayTable_clicked()
{
    ...
    std::vector<QStringList> originalData;
    loadCSV("C:/Users/LEOPARD/Documents/project_234 4/qt_data_norm_output.csv", normalizedData, originalData);
    int k = 12;
    KMeans kmeans(k);
    ...
    saveCSV("C:/Users/LEOPARD/Documents/project_234 4/qt_data_clust_output.csv", selectedHeaders, originalData, clusters);
}
